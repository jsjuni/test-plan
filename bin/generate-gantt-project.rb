...
class GenerateGanttProject < Logger::Application
  ...
  def cfg_duration(distance)
    (distance / 20.0 + 0.5).round
  end

  def obs_duration(distance)
    (distance / 10.0 + 0.5).round
  end
  ...
  def run
    ...
    cfg_cost_map = cost_map['scenarios']
    obs_cost_map = cost_map['observations']
    ...
    final_duration = cfg_duration((tests.last['scenarios']).inject(0) { |s, r| s + cfg_cost_map[r] })
